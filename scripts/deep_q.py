...
for i in range(num_iters):
    ...
    press(key)
    # time.sleep(.01)
    imageData = np.frombuffer(dataBitMap.GetBitmapBits(True), dtype=np.uint8).reshape((h, w, 4))
    release(key)
...
print("--- %s fps ---" % (num_iters/(time.time() - start_time)))
